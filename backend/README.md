...
同一导入/总资产账户现在可使用 `GET/HEAD/PUT /accounts/{id}/current-holdings` 维护手工现金和证券数量。`current_holdings` 只保存当前输入，版本审计进入 `audit_log`；不会写交易、外部资金流或总资产。已有交易账户继续重放，不接受手工持仓覆盖。配置后显式估值和周任务复用该快照，保存前精确校验来源版本；缺报价不保存小计。详情见 [API 第 13 节](docs/ledger-api.md#13-同账户当前持仓)。
...
这是与白酒独立的 worker，默认关闭；关闭时不建任务、不恢复任务状态、不请求报价。仅显式启用后，在周六北京时间 08:00（可通过上述时刻配置调整）至当日 24:00 的窗口内处理所有账户：有当前持仓来源的账户计算现金加持仓总资产，其它账户复制截至当日最近一笔有效总资产并标记沿用。只有从未记录总资产时才保存 `skipped/no_source`，不会伪造零资产。
...
- 仅完整、精确且来源/时间有效的结果可保存。采价期间相关账户持仓依据发生变更时 `failed/basis_changed`，不将已过期观察标成周更新成功。任务成功状态、history_id 和 schema-1 快照/水位同事务；同一任务重试不能重复记录。手动刷新同日仍追加，不按账户/日期全局去重。
