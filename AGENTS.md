...
账本持仓新增单股交易与自动现金分红（2026-09-25）：`/accounts/{id}/stock-book` 提供买卖、现金基准、单股成本收益与分红流水；新增迁移 005，保留旧数据。后台分红任务随账本服务运行，每分钟检查、市场数据缓存六小时，独立于 `LEDGER_WEEKLY_ENABLED=false`；GET/页面刷新仍只读。金额按公告口径，历史覆盖受来源限制，详见 [单股持仓说明](docs/specs/2026-09-25-stock-journal.md)。
