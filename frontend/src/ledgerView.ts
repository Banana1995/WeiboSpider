...
export const versionString = (v: unknown): v is string =>
  typeof v === "string" &&
  /^[1-9]\d{0,18}$/.test(v) &&
  BigInt(v) <= 9223372036854775807n;
const moneyValue = (v: unknown): v is string =>
  typeof v === "string" && decimal(v, 2);
...
export function validRecord(r: AccountRecord, accountID: string) {
  return !!(
    ...
    (r.sequence === undefined || versionString(r.sequence)) &&
    validDay(r.date) &&
    Object.hasOwn(accountRecordOriginLabels, r.origin) &&
    ...
    (r.kind === "asset"
      ...
      : r.kind === "cash_flow"
        ? r.flow !== null
        : r.kind === "log" && r.flow === null && r.total_assets === null) &&
    (!r.quote_audit_id || versionString(r.quote_audit_id))
  );
}
export function recordPage(
  value: Page<AccountRecord>,
  id: string,
  from: string,
  to: string,
  status: string,
  ...
  for (const r of value.items) {
    if (
      !validRecord(r, id) ||
      !versionString(r.sequence) ||
      seen.has(r.id) ||
      (from && r.date < from) ||
      (to && r.date > to) ||
      (status === "active" && r.voided) ||
      (status === "voided" && !r.voided) ||
      (previous &&
        (r.date > previous[0]! ||
          (r.date === previous[0] &&
            BigInt(r.sequence) >= BigInt(previous[1]!))))
    )
      throw new LedgerError("invalid_response");
  ...
  if (
    value.next_cursor &&
    (!last || value.next_cursor !== `${last.date}:${last.sequence}`)
  )
    throw new LedgerError("invalid_response");
  return value;
}
export function validSummary(s: EffectiveSummary) {
  return !!(
    ...
    [
      ...
      s.log_count,
      s.voided_count,
      s.latest_asset_count,
    ].every((v) => Number.isSafeInteger(v) && v >= 0) &&
    [s.from, s.to, s.latest_asset_date].every(
      (v) => v === null || validDay(v),
    ) &&
    typeof s.total_in === "string" &&
    /^\d+\.\d{2}$/.test(s.total_in) &&
    typeof s.total_out === "string" &&
    /^\d+\.\d{2}$/.test(s.total_out) &&
    nullableMoney(s.latest_assets) &&
...
export function validateBasis(
  ...
  const point = (p: BasisPoint) =>
    !!(
      p &&
      validDay(p.date) &&
      opaqueID(p.record_id) &&
      versionString(p.sequence) &&
      versionString(p.version) &&
      Object.hasOwn(pointLabels, p.status) &&
      typeof p.selected === "boolean" &&
      nullableMoney(p.assets) &&
      nullableMoney(p.flow) &&
      typeof p.source_id === "string" &&
      typeof p.source_version === "string" &&
      typeof p.source_date === "string" &&
      (p.source_id === ""
        ? p.source_version === "" && p.source_date === ""
        : opaqueID(p.source_id) &&
          versionString(p.source_version) &&
          validDay(p.source_date) &&
          p.source_date <= p.date) &&
      (!p.record ||
        (validRecord(p.record, account.id) &&
          p.record.id === p.record_id &&
          p.record.date === p.date &&
          p.record.version === p.version &&
          p.record.sequence === p.sequence &&
          p.record.flow === p.flow &&
          (p.record.total_assets === null ||
            (p.record.carried_from && !p.record.manual_assertion) ||
            p.assets === p.record.total_assets) &&
          !p.record.voided)) &&
      (!p.valuation ||
        (p.valuation.account_id === account.id &&
          p.valuation.id === p.record_id &&
          p.valuation.as_of === p.date &&
          p.valuation.currency === account.currency))
    );
  if (
    !b ||
    ...
    b.from !== (from || "0001-01-01") ||
    b.to !== to ||
    !validDay(b.to) ||
    typeof b.revision !== "string" ||
    !/^[a-f0-9]{64}$/.test(b.revision) ||
    typeof b.change_revision !== "string" ||
    !/^\d+$/.test(b.change_revision) ||
    ![
      "current",
      "unavailable",
    ...
    typeof b.previous_basis_affected !== "boolean" ||
    typeof b.net_flow !== "string" ||
    !/^-?\d+\.\d{2}$/.test(b.net_flow) ||
    !Array.isArray(b.points) ||
    b.points.length > 10000 ||
    b.points.some(
      (p, i) =>
        !point(p) ||
        p.date < b.from ||
        p.date > b.to ||
        (i > 0 &&
          (p.date < b.points[i - 1]!.date ||
            (p.date === b.points[i - 1]!.date &&
              BigInt(p.sequence) <= BigInt(b.points[i - 1]!.sequence)))),
    ) ||
    ![b.opening, b.closing].every(
      (p) => p === null || (point(p) && p.date <= b.to),
    ) ||
    !Array.isArray(b.changes) ||
    b.changes.length > 10000 ||
    b.changes.some(
      (c) =>
        !c ||
        !versionString(c.revision) ||
        !opaqueID(c.source_id) ||
        !versionString(c.source_version) ||
        !validDay(c.from) ||
        typeof c.reason !== "string",
    ) ||
    !validReturns(b.returns, b.revision, from, to)
  )
    throw new LedgerError("invalid_response");
  const selected = new Map<string, string>();
  const ids = new Set<string>();
  for (const p of b.points) {
    if (ids.has(p.record_id) || (p.selected && selected.has(p.date)))
      throw new LedgerError("invalid_response");
    ids.add(p.record_id);
    if (p.selected) selected.set(p.date, p.record_id);
  }
  if (
    b.returns.curve.some((p) =>
      p.baseline
        ? p.record_id !== b.returns.opening?.record_id
        : selected.get(p.date) !== p.record_id,
    ) ||
    [b.returns.opening, b.returns.closing].some(
      (p) => p !== null && (!point(p) || p.date > b.to),
    )
  )
    throw new LedgerError("invalid_response");
  ...
  if (
    b.returns.flows.some((f) => {
      const p = byID.get(f.record_id);
      return (
        !p || p.date !== f.date || p.version !== f.version || p.flow !== f.flow
      );
    })
  )
    throw new LedgerError("invalid_response");
