...
2026-09-12 开发工作树：账本采用单一账户模型，当前现金与持仓直接编辑，添加持仓时查询证券身份，不再单独登记证券或回放交易。顶部“记一笔”维护独立的转入、转出和固定总资产；原始导入、估值依据和审计保留。个人年化使用后端 XIRR，不使用原型示例计算。当前操作说明见 [账本界面](frontend/README.md#正式账本界面)。

账本总资产与所有收益指标统一采用最近明确资产加后续日期净外部资金流，同日明确总额已含当日全部资金流；原始空资产仍为空。参考行情只读，手动保存行情入口已移除；周任务保存固定资产与不可变估值证据，持仓修改不使历史失效。**此重构仅在本地实现与离线验证，未提交、推送、部署或访问生产数据，未运行 E2E/真实来源；不代表上方生产入口已运行此版本。** fresh schema 无旧库迁移或兼容回退，不自动清库。详见 [当前模型及验证](docs/specs/2026-09-12-ledger-current-inputs.md) 和 [当前持仓 API](backend/docs/ledger-api.md#13-同账户当前持仓)。
