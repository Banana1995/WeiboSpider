...
func TestFXRefetchDoesNotRewriteSnapshots(t *testing.T) {
	...
	manualSourceAccount(t, s, "cny")
	require.NoError(t, s.AddInstrument(t.Context(), Instrument{ID: "usd-stock", Market: "SH", Code: "900001", Name: "Synthetic", Currency: USD}))
	putSource(t, s, "cny", "current", "0", 10000, CurrentPosition{"usd-stock", 1000000})
	first := sampleValuation(t, s, "cny", Handler{Quotes: valuationQuotes(validValuationQuotes), FX: p})
	frozen, err := s.ValuationHistory(t.Context(), "cny", mustInt(t, first.HistoryID))
	require.NoError(t, err)
	price.Store("7.2")
	second, err := p.Fetch(t.Context(), FXRequest{Base: USD, Quote: CNY, Mode: "latest"})
	require.NoError(t, err)
	require.NotEqual(t, first.Items[0].FX.Rate, second.Rate)
	after, err := s.ValuationHistory(t.Context(), "cny", mustInt(t, first.HistoryID))
	require.NoError(t, err)
	require.Equal(t, frozen, after)
	changed := sampleValuation(t, s, "cny", Handler{Quotes: valuationQuotes(validValuationQuotes), FX: p})
	require.NotEqual(t, *first.TotalAssets, *changed.TotalAssets)
	after, err = s.ValuationHistory(t.Context(), "cny", mustInt(t, first.HistoryID))
	require.NoError(t, err)
	require.Equal(t, frozen, after)
}
