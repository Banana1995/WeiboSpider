...
新增 `/portfolios` 定义管理、组合分析及年度收益接口。组合在同一数据库快照中合并成员资产与资金流，复用 Modified Dietz/XIRR/TWR 计算；支持晚开始的成员、期初资产带入、流量调整沿用和成员贡献核对。组合使用独立定义，来源仍是原账户有效记录。

本次新增 `003_portfolios.sql`，从当前已应用 `001_init.sql`、`002_account_deletion.sql` 的账本增量升级，保留账户、记录和幂等回执。详情见 [API 第 14 节](docs/ledger-api.md#14-账户组合) 和[设计与验证](../docs/specs/2026-09-24-ledger-portfolios.md)。
...
2026-09-12 的当前持仓重构曾按当时授权直接收敛 `001_init.sql`。这是历史开发边界；后续账户删除与组合分别使用 `002`、`003` 增量迁移。迁移校验和检查仍拒绝不匹配的更早结构，不自动清除任何数据库。单元测试只使用临时合成库。
