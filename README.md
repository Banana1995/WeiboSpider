...
正式账本前端按批准的 `/ledger-prototype` 重做：顶部统一“记一笔”（转入 / 转出 / 更新总资产），收益概览下方常驻一张账户记录表；账户资料、当前持仓、持仓交易、Excel 导入和自动更新状态收进“管理账户”。正式页面只使用真实 API，个人年化使用后端 XIRR，不使用原型示例计算。原始导入、估值依据和审计仍可通过 API 查询，不在日常主界面堆叠展示。**本轮类型检查和生产构建通过，按授权推送发布；部署结果以对应 GitHub Actions 为准，功能与浏览器验收由用户执行。未运行单元测试、完整测试套件、E2E 或独立审查。** 当前操作说明见 [账本界面](frontend/README.md#正式账本界面)。

开发工作树新增同账户“当前持仓”：账户级 Excel 历史保持不变，可另行维护当前现金和证券数量，无需新账户、切换日期或补录交易。保存持仓只更新估值来源；预览不入账，显式“更新并保存总资产”才追加同账户总资产。该改动尚未部署，直接修改开发阶段 fresh schema，不提供旧库迁移或兼容承诺，也不会自动清库。详见 [当前持仓 API](backend/docs/ledger-api.md#13-同账户当前持仓)。
